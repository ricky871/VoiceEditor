...
def safe_infer(
    ...
    *,
    ...
    Logs all errors and raises the last exception if all candidates fail.
    """
    last_exc = None
    last_exc_type = None
    
    for attempt_idx, attempt in enumerate(duration_candidates, start=1):
        kwargs = {**base_kwargs, **attempt}
        try:
            logging.debug(
                "Segment %d inference attempt %d/%d with kwargs: %s",
                segment_index,
                attempt_idx,
            ...
            if key in message:
                last_exc = exc
                last_exc_type = "TypeError"
                logging.debug(
                    "Segment %d: Duration key %s unsupported, trying fallback. Error: %s",
                    segment_index,
                    key,
                    message,
                )
                continue
            # Re-raise TypeError if it's not related to our duration key
            logging.error(
                "Segment %d: TypeError not related to duration key: %s",
                segment_index,
                message,
            )
            raise
            ...
            logging.warning(
                "Segment %d inference attempt %d failed with %s: %s. Trying next candidate...",
                segment_index,
                attempt_idx,
                last_exc_type,
                str(exc)[:200],  # Limit error message length
            )
            if attempt_idx == len(duration_candidates):
                # Last attempt, log with error level
                logging.error(
                    "Segment %d: All inference attempts failed. Last error type: %s, message: %s",
                    segment_index,
                    last_exc_type,
                    str(exc),
                )
...
def process_final_audio(
    ...
    final_audio_path = config.out_dir.parent / "final.wav"
    ensure_dir(final_audio_path.parent)
    final_audio.export(str(final_audio_path), format="wav")
    logging.info(
        "Stitched output written to %s | duration %.2f s",
        final_audio_path,
    ...
    if video_source and video_source.exists():
        default_video_out = srt_path.parent / f"{video_source.stem}_dubbed{video_source.suffix}"
        ...
        try:
            ...
            logging.info("Muxed video written to %s", output_video)
        except RuntimeError:
            return None
